...
<body>
   ...
   영화제목
   <br>
   <span id="movieName">${tcInfo.mv_name}</span>
   <br>
   <br> 포맷
   <br>
   <span id="theaterKind">thKind</span>
   <br> 상영일
   <br> msSdate
   <br> 상영시간
   <br> msStime~msEtime
   <br> 상영관
   <br>
   <span id="theaterName">상영관</span>
   <span id="theaterNum"></span>관
   <br> ${list } [url]
   <br>	성인 : ${adult } , 청소년 : ${youth }, 우대 : ${special } [url]
   ...
   */
   ...
   var specialNum = 1;
    ...
    function setupPage(theater) {
     ...
     var totalAmount = adult*adultNum + teen*teenNum + special*specialNum;
...
const checkboxes = document.querySelectorAll('.chk');
      let checkedCheckboxes = document.querySelectorAll('.chk:checked'); // 초기에 체크된 상태를 가져옴// 전역 범위에서 선언된 변수
      
checkboxes.forEach((checkbox, index) => {
    checkbox.addEventListener('change', function() {
        // 현재 체크된 체크박스 수 계산
        checkedCheckboxes = document.querySelectorAll('.chk:checked'); // 전역 변수에 할당
        if (checkedCheckboxes.length > adultNum + teenNum + specialNum) { // 인원수 합산(0910)
 ...
 */
      ...
      //var newAmount = ${ticketPrice.tp_price};
      var cDiscount = 0;
      ...
      $("#modal_submit_btn").click(function() {
          var selectCouponList = []; // 선택된 쿠폰 코드를 저장할 배열
          ...
          $("#cchk:checked").each(function() {
             ...
             if (selectCouponList.length > 0) { // 적어도 하나 이상의 쿠폰이 선택되었을 때
                 $.ajax({
                     type: "POST",
                     url: "/couDiscount",
                     data: JSON.stringify(selectCouponList), // 배열을 JSON 형태로 변환하여 전송
      ...
      var selectAdm = [];
       ...
       if (selectAdm.length > 0) {
           ...
           if (teenNum > 0) {
               for (let i = 0; i < Math.min(teenNum, selectAdm.length - aDiscount / teen); i++) {
                   aDiscount += teen;
               }
           }
           if (specialNum > 0) {
       ...
       newAmount2 = newAmount - aDiscount; // 티켓가격으로 변경
      ...
      newAmount2 = newAmount - aDiscount;
       ...
       $("#cardAdd").click(function() {
              $("#modal_change1").hide(); // modal_change1 숨기기
                $("#modal_change2").show(); // modal_change2 보이기
         });
      ...
      //간편 결제 카드 추가 버튼//
       $("#modal_submit_btn6").click(function(){
          ...
          const agree = document.querySelector('.chk4');
       ...
       //간편결체창 등록카드 목록
         $("#modal_open_btn5").click(function() {
            $("#modal_change1").attr("style", "display:block");
